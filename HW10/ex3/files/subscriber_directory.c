...
void remove_abonent(abonent_list *list)
{
    ...
    {
        abonent_node *next_ab = current_ab->next; // Сохраняем ссылку на следующий

        if(string_equality(current_ab->name, a_name))
        {
            flag++;

            if(current_ab->prev != NULL)
                current_ab->prev->next = current_ab->next;
            else
                list->head = current_ab->next; // Удаляем первый элемент

            if(current_ab->next != NULL)
                current_ab->next->prev = current_ab->prev;
            else
                list->tail = current_ab->prev; // Удаляем последний элемент

            printf("Удалён абонент: %s  %s\n", current_ab->name, current_ab->second_name);
            free(current_ab);
        }
        current_ab = next_ab;
    }
